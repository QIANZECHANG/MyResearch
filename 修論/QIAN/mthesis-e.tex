...
\tikzstyle{code} = [draw, ellipse,fill=red!20, text width=3.2em, minimum height=3em]
...
\begin{tikzpicture}[node distance = 3.5cm, on grid]
  ...
  \node[code, below of=infer,yshift=1.5cm] (src){input.c};
  ...
  \node[code, above of=fuzzer2,yshift=-1.5cm] (output){output.c};
...
\vspace{0.4cm}
\hspace{-0.7cm}\fbox{\begin{minipage}{\textwidth}
HAMER can handle multiple returns, multiple errors, and collect critical tests when \textcolor{red}{LibFuzzer detects the errors successfully}.
\end{minipage}}

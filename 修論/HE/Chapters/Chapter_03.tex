...
\cleardoublepage
Ozeki et al., (2018)~\cite{ref45} explored disaster prevention consciousness can be expressed by 24 elements in knowledge, behavior, and psychology. This survey selected disastrous imagination, sense of crisis, others-oriented, anxiety, and apathy about disasters to express disaster prevention consciousness. The detailed descriptions of each question are shown in Table~\ref{table2item2}. All answers are based on a scale of 1 to 6, detailed meanings are not at all applicable, mostly not applicable, somewhat not applicable, somewhat agree, mostly agree, strongly agree. 
...
\begin{table}[h]
  ...
  \centering
  \begin{tabular}{l|c|c}
               &  \begin{tabular}{c}  Staying in a\\tourist attrbehavior \end{tabular} & \begin{tabular}{c}  Moving by public\\transportation \end{tabular}  \\
    \hline
...
In order to answer response behaviors during the disaster, all respondents were requested to watch a simulation video of the 'Tokyo Metropolitan Earthquake'. The video is provided by the Cabinet Office, the survey was translated and distributed in their native languages. The simulation video~\footnote{\url{http://wwwc.cao.go.jp/lib_012/syuto_02.html}} shows an earthquake of magnitude 7.3 that occurred in the southern part of Tokyo, which happened on a winter evening in the year 20xx, and in order to show the damage, the landscape is represented brighter than it actually is. After watching the video, the respondents were requested to select 5 response behaviors with an order in each of the scenarios.

In the pre-survey, we collected the data on evacuation behaviors of foreign visitors and Japanese. Then, we utilized the pre-survey's results and set these behaviors as choices. It's worth noting that choices 1-7 are only available in scenarios 1 and 3 because they require a phone/internet connection. Table~\ref{table4} contains a list of all choices as well as a detailed description of each one.

%%%%%%%%%%%%%%%%%%%%%
%\iffalse
\begin{table}[h]
  \caption{Available choices of behaviors}
  \label{table4}
  \centering
  \begin{tabular}{c|l}
  \hline
  Choice & Actions - Information seeking behavior \\
  \hline

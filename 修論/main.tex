 ...
 \ENSURE  SynInf
  ...
  \WHILE {$i < 10$}
   \STATE $res \gets \textbf{Fuzz}(inst\_code)$
    \FOR {$errID = 0$ to $|Dep|-1$}
     \FOR {$var \in dep\_var$}
     \IF {$memory$ $leak$ $happen$}
      \STATE $SynInf[errID][var].add((res[var],1))$
     \ELSE
      \STATE $SynInf[errID][var].add((res[var],0))$
     \ENDIF
     \ENDFOR
    \ENDFOR
   \STATE $i\gets i + 1$
...
\subsection{Repair Algorithm}
 ...
 \ENSURE  fixed\_code
  ...
  \STATE $flag = 0$
  \WHILE {$|queue| \ne 0 $ and $flag \ne |queue| $}
   \STATE $errID \gets queue.pop$
   \STATE $err\_inf \gets SynInf[errID]$
   \REPEAT
